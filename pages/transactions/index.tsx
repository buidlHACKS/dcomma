...
import { reqTransactions } from "../../services/httpReqDcommas";
import {
  countTotalFees,
  formatBalance,
    getMappingCurrency,
...
const mapRes = (res) => {
  return res.map((item) => {
    return {
      ...
      successful: item?.status !== undefined ? item.status : true,
      log_events: item?.transaction_tags || [],
      deployed_contract: item?.deployed_contract || null,
      gas_price: item?.gas_price || '',
      gas_used: item?.gas_used || '',
      gas_quote_rate: item?.block_base_fee_per_gas || ''
    };
  });
};
