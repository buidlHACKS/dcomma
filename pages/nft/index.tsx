...
import { reqAllNFTs ,reqNFTAddress } from "../../services/httpReqDcommas";
import { getMappingChains } from "../../utils";
...
export interface NFTGetcontractAddress{
...
export const IndexPage = () => {
  ...
  const [NFTItems, setINFTItems] = useState<NFTGetcontractAddress[]>([]);
  ...
  useEffect(() => {
    ...
    //     console.log("nft data response ======>>>>>>>",response?.result || [])
    //   } )
    reqNFTAddress( address ).then( response =>{
          
        setINFTItems( response?.result || [] )
        
        console.log("Address data response",response?.result || [])
        })
      .finally(() => {
        setIsLoading(false);
      });
              ...
              {NFTItems?.map((item, index) => {
                return (
                  <tr
                    ...
                    onClick={() => {
                      router.push(`/nft/${item.contract_address}`);
                    }}
